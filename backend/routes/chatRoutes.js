...
router.post('/zoho', async (req, res) => {
  try {
    ...
    if ((q.includes('overdue task') || q.includes('pending task') || q.includes('task') || q.includes('my task')) && !q.includes('create task') && !q.includes('add task') && !q.includes('new task') && !q.includes('delete task') && !q.includes('remove task') && !q.includes('cancel task')) {
      const now = new Date();
