...
export default function AssetDetailPage() {
  ...
  const fetchAssetDetails = () => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/assets/${params.id}`)
      .then(res => res.json())
      .then(data => {
        if (data && data.data) {
          setAsset(data.data);
  ...
  const handleStatusUpdate = async (status: string) => {
    if (!asset) return;
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/assets/${asset.id}/internal-status`, {
        method: 'PATCH',
  ...
  const handleUploadVersion = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/assets/${asset.id}/versions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          file_url: newFileUrl,
