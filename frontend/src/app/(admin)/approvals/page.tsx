...
export default function ApprovalsPage() {
  ...
  const fetchAssets = () => {
    fetch((process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com') + '/api/assets')
      .then(res => res.json())
      .then(data => {
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com') + '/api/assets', {
        method: 'POST',
