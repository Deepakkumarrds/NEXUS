      ...
      if (response.ok) {
        ...
        if (result.data && result.data.months) {
            for (const month of result.data.months) {
                await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com'}/api/sows/month/${month.id}/submit`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
        ...
        alert('SOW Created and Submitted for Approval!');
        // Reset form
        setFormData({ client_id: '', sow_name: '', start_date: '', end_date: '' });
        setSowMonths([]);
      } else {
    ...
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-sm border border-slate-200">
      <div className="space-y-6 text-sm">
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            <label className="block font-medium text-slate-700 mb-1.5">Client (Brand)</label>
            <select 
              name="client_id"
              required
        ...
        <div className="pt-6 border-t border-slate-100">
          <div className="mb-4">
            <h3 className="font-semibold text-slate-900">Monthly Deliverables</h3>
            <p className="text-xs text-slate-500 mt-1">Define the work for each month. These will be sent for approval.</p>
          </div>
