...
export default function EscalationsPage() {
  ...
  const fetchEscalations = () => {
    fetch((process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com') + '/api/escalations')
      .then(res => res.json())
      .then(data => { 
        if (data && data.data) {
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com'}/api/escalations/${id}/status`, {
        method: 'PATCH',
  ...
  const deleteEscalation = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com'}/api/escalations/${id}`, {
        method: 'DELETE'
      });
