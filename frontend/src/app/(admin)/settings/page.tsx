...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com'}/api/settings`)
      .then(res => res.json())
      .then(data => {
        if (data && data.data) {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com'}/api/settings/delivery-logs`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com'}/api/settings`, {
        method: 'PUT',
        headers: {
  ...
  const handleTrigger = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com'}/api/settings/trigger-report`, {
        method: 'POST',
  ...
  const handlePreview = async (clientId: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com'}/api/settings/preview-report/${clientId}`);
      if (res.ok) {
