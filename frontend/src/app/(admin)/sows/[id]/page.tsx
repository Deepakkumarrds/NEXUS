...
export default function EditSowPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch((process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com') + `/api/sows/${id}`).then(res => res.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'https://nexus-p3l0.onrender.com') + `/api/sows/${id}`, {
        method: 'PUT',
