...
export default function TaskDetailsPage() {
  ...
  const fetchTaskDetails = () => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com'}/api/tasks/${taskId}`)
      .then(res => res.json())
      .then(data => {
        if (data && data.data) {
          setTask(data.data);
  ...
  const updateTaskData = async (updates: any) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com'}/api/tasks/${taskId}`, {
        method: 'PUT',
  ...
  const cloneTask = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com'}/api/tasks/${taskId}/clone`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handlePostComment = async (e: React.FormEvent) => {
    ...
    const user = JSON.parse(localStorage.getItem('user') || '{}');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com'}/api/tasks/${taskId}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
