...
function NewTaskForm() {
  ...
  useEffect(() => {
    ...
    fetch((process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com') + '/api/holidays')
      .then(res => res.json())
      .then(data => { if(data && data.data) setHolidays(data.data); });

    // Fetch SOWs
    fetch((process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com') + '/api/sows')
      .then(res => res.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'https://rds-db.onrender.com') + '/api/tasks', {
        method: 'POST',
