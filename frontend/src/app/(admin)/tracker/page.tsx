...
export default function TrackerPage() {
  ...
  const fetchTrackerData = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/tracker?department=${encodeURIComponent(department)}&startDate=${startDate}&endDate=${endDate}`, {
  ...
  const saveCell = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5000/api/tracker/cell`, {
        method: 'POST',
        headers: {
