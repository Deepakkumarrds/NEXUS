...
export default function ClientApprovalsPage() {
  ...
  const fetchAssets = (clientId: string) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/assets/client/${clientId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSaveAnnotation = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/assets/versions/${activeVersion.id}/annotations`, {
        method: 'POST',
  ...
  const handleStatusUpdate = async (status: string) => {
    ...
    setIsSubmitting(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/assets/${selectedAsset.id}/client-status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
            ...
            <div key={asset.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
              <div className="h-56 bg-slate-100 relative flex items-center justify-center border-b border-slate-200 group">
                {latestVersion && isImage(latestVersion.file_type) ? (
                  <img src={(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000') + latestVersion.file_url} alt={asset.title} className="object-cover w-full h-full" />
                ) : (
                ...
                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                  <button 
                    onClick={() => {
                      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/assets/${asset.id}`)
                        .then(res => res.json())
                        .then(data => {
              ...
              <div className="flex-1 relative overflow-auto p-4 flex items-center justify-center min-h-[500px]">
                {isImage(activeVersion.file_type) ? (
                  <div className="relative inline-block shadow-md cursor-crosshair">
                    <img 
                      ref={imageRef}
                      src={(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000') + activeVersion.file_url} 
                      alt="Asset" 
                  ...
                  <div className="text-center text-slate-500">
                    ...
                    <a href={(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000') + activeVersion.file_url} target="_blank" className="text-indigo-600 underline mt-2 inline-block">Download File</a>
                  </div>
                )}
              </div>
            </div>
